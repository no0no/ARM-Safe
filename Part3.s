...
@ Extra: Display LED board message during each part
	...
	@ Detailed message during error (E) 
		...
		@ Error if wrong password is set during confirmation
...
@ ==== (2) Black Buttons ==== @
.equ LEFT_BLACK_BUTTON, 			0x02 					@ Left Black Button
.equ RIGHT_BLACK_BUTTON, 			0x01 					@ Right Black Button
...
SafeMain:													@ Basically reset all registers 
	mov r8, #0			
	mov r3, #0			
	mov r5, #0			
	mov r4, #0			
...
PrevNumbers:												@ Allow first button pressed from SafeMain to go through
	swi SWI_CheckBlue
...
Numbers:													@ Whichever # is pressed, branch to that number function 
	...
	beq Fifteen
	cmp r8, #8
	beq SafeMain												@ Check to see if another # is pressed after
...
Less4:
	bal SafeMain 												@ Less than 4, start again 
...
LeftBlackLock:												@ Lock the safe if everything is right 
	...
	mov r7, #1												@ Lock safe 
	bal SafeMain												@ Branch to beginning
